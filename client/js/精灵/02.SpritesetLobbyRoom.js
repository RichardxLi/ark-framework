...
function SpritesetLobbyRoom(viewport, lobby) {
    ...
    this.dispose = function() {
        for(var i=0; i<_spriteRooms.length; i++) {
            if(_spriteRooms[i]!=null) _spriteRooms[i].dispose();
        }
    };

    var clearall = function() {
        for(var i=0; i<_spriteRooms.length; i++) {
            if(_spriteRooms[i]!=null) _spriteRooms[i].clearBitmap();
        }
    };

    var drawRoom = function(i) {
        if(_spriteRooms[i] == null) {
            var bitmap = new IBitmap.CBitmap(750,150);
            _spriteRooms[i] = new ISprite(bitmap, viewport);
        }
        _spriteRooms[i].drawRect(boardRect(), IColor.Black());
        var color = openColor();
        if(_lobby.rooms[i].state == 1) {
            color = closeColor();
        }
        _spriteRooms[i].drawRect(contentRect(), color);
        _spriteRooms[i].drawTextQ(_lobby.rooms[i].name, 27, 32,IColor.Black(),60);
        _spriteRooms[i].drawTextQ(_lobby.rooms[i].name, 25, 30,IColor.White(),60);
        _spriteRooms[i].x = i%2*860 + 30;
        _spriteRooms[i].y = parseInt(i/2)*170;
    };
